    ...
    <{if $page eq 0}>
        <div class="article-section-container" style="padding: 0 20px;">
            ...
            <div class="article-list">
                <{if $article.writer}>
                    <div>
                        <span class="article-label"><{php}>echo art_constant("MD_AUTHOR");<{/php}>:</span><span
                                class="article-content"><a href="#article-writer"><{$article.writer.name}></a></span>
                    </div>
                <{/if}>

                <{if $article.source}>
                    <div>
                        <span class="article-label"><{php}>echo art_constant("MD_SOURCE");<{/php}>:</span><span
                                class="article-content"><{$article.source}></span>
                    </div>
                <{/if}>

                <{if $article.summary}>
                    <div class="article-summary"><span
                                class="article-label"><{php}>echo art_constant("MD_SUMMARY");<{/php}>:</span><span
                                class="article-content"><{$article.summary}></span></div>
                <{/if}>

                <{if $tagbar}>
                    <!--
                        There are two ways to render tags:
                        1. Simply use the tag_bar.tpl template
        ...
        <div class="article-section-container">
            <form action="<{$xoops_url}>/modules/<{$xoops_dirname}>/action.rate.php" method="post">
                ...
                <div class="rate-item">
                    <{php}>
                        for($i=10;$i>0;$i--) echo'
                        <span><input type="radio" name="rate" id="rate'.$i.'" value="'.$i.'">'.$i.'</span>
                        ';
                    <{/php}>
                    <span>
...
<div id="article-api" class="article-section article-api">
    ...
    <{if $transfer}>
        ...
        <a href="<{$xoops_url}>/modules/<{$xoops_dirname}>/transfer.php<{$smarty.const.URL_DELIMITER}>c<{$article.category}>/<{$article.id}>/<{$op}>"
           target="<{$op}>"
           title="<{$opt.desc}>"><{$opt.title}></a>
    <{/foreach}>
        |
    <{/if}>

    <a href="<{$xoops_url}>/modules/<{$xoops_dirname}>/xml.php<{$smarty.const.URL_DELIMITER}>rss/c<{$article.category}>/<{$article.id}>"
       target="api"><{php}>echo art_constant("MD_RSS");<{/php}></a>
    |
    <a href="<{$xoops_url}>/modules/<{$xoops_dirname}>/xml.php<{$smarty.const.URL_DELIMITER}>rdf/c<{$article.category}>/<{$article.id}>"
       target="api"><{php}>echo art_constant("MD_RDF");<{/php}></a>
    |
    <a href="<{$xoops_url}>/modules/<{$xoops_dirname}>/xml.php<{$smarty.const.URL_DELIMITER}>atom/c<{$article.category}>/<{$article.id}>"
