...
function [VAR_PREFIX]_block_news_show( $options )
{
    ...
    foreach (array_keys($categories_obj) as $id) {
        if (!$articles_category = $categoryHandler->getLastArticleIds($categories_obj[$id], $options[1])) continue;
    ...
    foreach (array_keys($categories_obj) as $id) {
        ...
        $articles_category_id = $categoryHandler->getLastArticleIds($category, $options[1]);
