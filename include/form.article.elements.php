...
// editor form mode
if ('fix' === @$helper->getConfig('form_mode')) {
...
if ($canupload) {
    ...
    if (!empty($art_image_file_tmp)) {
        $image_option_tray->addElement(new \XoopsFormLabel('', '<a href="' . XOOPS_URL . '/' . $helper->getConfig('path_image') . "/{.$art_image_file_tmp}\" rel=\"external\">" . art_constant('MD_IMAGE_UPLOADED') . '</a>'));
...
foreach (array_keys($top_categories) as $id) {
    ...
    $sub             = empty($top_categories[$id]['sub']) ? [] : $top_categories[$id]['sub'];
    ...
    foreach ($sub as $sid => $scat) {
        ...
        if ($categoryHandler->getPermission($sid, 'moderate')) {
            $category_string .= '<input type="checkbox" name="category[]"';
...
if ($topics_obj = $topicHandler->getByCategory($allowed_cats, $helper->getConfig('topics_max'), 0, $criteria, $tags)) {
    foreach ($topics_obj as $top_id => $top) {
        if ($topicHandler->getPermission($top, 'moderate')) {
...
// Forum
if (!empty($helper->getConfig('forum')) && $isModerator && !$article_obj->getVar('art_forum')) {
...
// Trackbacks
if (!empty($helper->getConfig('do_trackback'))) {
...
// Notify for approval
if (!$canPublish && is_object($xoopsUser) && $helper->getConfig('notification_enabled')) {
