...
if (empty($start)) {
    ...
    $cats         = empty($category_id) ? array_keys($categories_array) : [$category_id];
    ...
    if (empty($month)) {
        $sql    = '    SELECT '
                  ...
                  . ' AS a '
                  . '        LEFT JOIN '
                  . art_DB_prefix('artcat')
                  . ' AS ac ON ac.art_id = a.art_id '
                  . '    WHERE '
...
if (!empty($category_obj)) {
    $xoopsTpl->assign('category', ['id' => $category_id, 'title' => $category_obj->getVar('cat_title')]);
}
