...
class PermissionHandler extends \XoopsGroupPermHandler
{
    ...
    public function getPermission($gperm_name = 'access', $id = 0)
    {
        ...
        if (!$allowed_groups = $this->getGroups($gperm_name, $id)) {
            return false;
        }
    ...
    public function &getCategories($perm_name = 'access')
    {
        ...
        $allowed_cats = [];
        if (is_array($_cachedPerms) && count($_cachedPerms)>0) {
    ...
    public function &getGroups($gperm_name, $id = 0)
    {
        ...
        $groups       = empty($_cachedPerms[$id]) ? [] : $_cachedPerms[$id];
        unset($_cachedPerms);

        return $groups;
    }

    public function createPermData($perm_name = '')
    {
        global $xoopsModule;
            ...
            foreach (array_keys($glist) as $i) {
                ...
                foreach ($ids as $id) {
                    foreach ($GLOBALS['perms_global'] as $permname => $perm_info) {
                        if (!empty($perm_name) && $perm_name != $permname) {
                            continue;
                        }
                        if ($perm_info['id'] == $id) {
                            $perms[$permname][0][] = $i;
        ...
        if (empty($perm_name) || 'all' === $perm_name || in_array($perm_name, array_keys($GLOBALS['perms_category']))):
            foreach (array_keys($GLOBALS['perms_category']) as $permname) {
                if (!empty($perm_name) && 'all' !== $perm_name && $perm_name != $permname) {
                    continue;
                }
                foreach (array_keys($glist) as $i) {
                    ...
                    foreach ($cats as $cat) {
                        ...
                        if (empty($perm_name) || 'all' === $perm_name) {
                            $perms['all'][$cat][] = $i;
                        }
                    }
                }
    ...
    public function &loadPermData($perm_name = 'access')
    {
        load_functions('cache');
    ...
    public function applyTemplate($category, $mid = null)
    {
        $perm_template = $this->getTemplate();
        if (empty($perm_template)) {
            return false;
        }
