...
class CategoryHandler extends \XoopsPersistableObjectHandler
{
    ...
    {
        parent::__construct($db, art_DB_prefix('category', true), Category::class, 'cat_id', 'cat_title');
    }
     ...
     */
    public function &getLastArticleIds(&$category, $limit = 0)
    {
        $art_ids = $category->getVar('cat_lastarticles');
        if ($limit > 0) {
            @array_splice($art_ids, $limit);
        }
     ...
     */
    public function setLastArticleIds($cat_id = null)
    {
        ...
        } elseif ($cat_id = (int)$cat_id) {
            $cat = &$this->get($cat_id);
            $this->_setLastArticleIds($cat);
            unset($cat);
        } else {
            $cats = $this->getAllByPermission('access', ['cat_id']);
    ...
    public function _setLastArticleIds(&$category)
    {
     ...
     *
     * @param \XoopsObject $category {@link Xcategory}
     *                               reference to Xcategory
     ...
     * @return bool   true on success
     */
    public function delete(\XoopsObject $category, $force = true, $forceDelete = false)
    {
        $des_cat        = $category->getVar('cat_pid') ?: 0; // move to parent category
        $articleHandler = \XoopsModules\Article\Helper::getInstance()->getHandler('Article', $GLOBALS['artdirname']);
        $articles       = $articleHandler->getByCategory($category);

        if (!empty($forceDelete) || empty($des_cat)) {
            foreach (array_keys($articles) as $id) {
                $articleHandler->terminateCategory($articles[$id], $category->getVar('cat_id'));
                if (is_object($articles[$id]) && $articles[$id]->getVar('cat_id') != $category->getVar('cat_id')) {
                    $articleHandler->updateCategories($articles[$id]);
                }
            }
        } else {
    ...
    public function &getByArticle($art_id, $criteria = null)
    {
        ...
        if (empty($orderSet)) {
            $sql .= ' ORDER BY c.cat_id, c.cat_order';
        }
     ...
     * @return array  of categories {@link Xcategory}
     */
    public function &getAllByPermission($permission = 'access', $tags = null)
    {
        $ret = [];
     ...
     * @return array  of categories {@link Xcategory}
     */
    public function &getChildCategories($category = 0, $permission = 'access', $tags = null)
    {
        $pid = (int)$category;
        $ret = [];
     ...
     * @return array  of categories {@link Xcategory}
     */
    public function &getSubCategories($pid = 0, $permission = 'access', $tags = null)
    {
        $pid         = (int)$pid;
        ...
        $tree           = new Article\Tree($categories);
     ...
     * @return array  of categories {@link Xcategory}
     */
    public function &getSupCategories(&$category, $reverse = true)
    {
        if (!is_object($category)) {
            $category = &$this->get((int)$category);
        }
    ...
    public function _getSup(&$category_array, $id = 0)
    {
        if (empty($id)) {
            return null;
        }
        $category         = $this->get((int)$id);
        $category_array[] = (int)$id;
        $pid              = $category->getVar('cat_pid');
        unset($category);
        $this->_getSup($category_array, $pid);
     ...
     * @return bool   true on success
     */
    public function updateTracks(&$category, $tracks = null)
    {
        if (null === $tracks) {
            $tracks = $this->getSupCategories($category);
        }
     ...
     * @return bool   true on success
     */
    public function setTrack(&$category, $ids = [])
    {
        if (!is_array($ids)) {
            return false;
        }
     ...
     */
    public function &getTrack(&$category, $incCurrent = false)
    {
        $ret = [];
        if (!is_object($category)) {
            return $ret;
     ...
     */
    public function &getTree($pid = 0, $permission = 'access', $prefix = '--', $tags = [])
    {
        $pid         = (int)$pid;
     ...
     * @return array   associative array of category IDs and sanitized titles
     */
    public function &getArrayTree($pid = 0, $permission = 'access', $tags = null, $depth = 0)
    {
        $pid         = (int)$pid;
        $perm_string = $permission;
        if (!is_array($tags) || 0 == count($tags)) {
     ...
     */
    public function &getArticles(&$category, $limit = 0, $start = 0, $criteria = null, $tags = null, $asObject = true)
    {
     ...
     */
    public function getArticleCountFeatured(&$cat_id, $criteria = null)
    {
        if (isset($criteria) && is_subclass_of($criteria, 'criteriaelement')) {
     ...
     * @return int    count of articles
     */
    public function getArticleCountPublished(&$cat_id, $criteria = null)
    {
        if (isset($criteria) && is_subclass_of($criteria, 'criteriaelement')) {
     ...
     * @return int    count of articles
     */
    public function getArticleCountRegistered(&$cat_id, $criteria = null)
    {
        if (isset($criteria) && is_subclass_of($criteria, 'criteriaelement')) {
     ...
     * @return int    count of articles
     */
    public function getArticleCount(&$cat_id, $criteria = null)
    {
     ...
     * @param object $criteria {@link CriteriaElement} to match
     * @return array count of articles
     */
    public function getArticleCountsRegistered($cat_id = 0, $criteria = null)
    {
        if (isset($criteria) && is_subclass_of($criteria, 'criteriaelement')) {
            $criteria->add(new \Criteria('ac.ac_register', 0, '>'));
        } else {
     ...
     */
    public function &getArticleCounts($cat_id = null, $criteria = null)
    {
     ...
     */
    public function &getCategoryCounts($cat_pid = 0, $permission = 'access')
    {
     ...
     */
    public function getPermission(&$category, $type = 'access')
    {
    ...
    public function cleanOrphan($table_link = null, $field_link = null, $field_object = null)
    {
        ...
        if (version_compare(mysqli_get_server_info(), '4.1.0', 'ge')) :
            $sql = 'DELETE ' . $this->table . ' FROM ' . $this->table . ' LEFT JOIN ' . $this->table . ' AS aa ON ' . $this->table . '.cat_pid = aa.cat_id ' . ' WHERE ' . $this->table . '.cat_pid>0 AND (aa.cat_id IS NULL)';
        else:
            ...
            if ($parent_categories = @array_values($category_list)) {
                ...
                foreach ($category_list as $cat_id => $parent_category) {
                    if (in_array($parent_category, $parent_list)) {
                        continue;
                    }
        ...
        /* for MySQL 4.1+ */
        if (version_compare(mysqli_get_server_info(), '4.1.0', 'ge')):
            $sql = 'DELETE FROM ' . art_DB_prefix('artcat') . ' WHERE (cat_id NOT IN ( SELECT DISTINCT cat_id FROM ' . $this->table . ') )';
        else:
            $sql = 'DELETE ' . art_DB_prefix('artcat') . ' FROM ' . art_DB_prefix('artcat') . ' LEFT JOIN ' . $this->table . ' AS aa ON ' . art_DB_prefix('artcat') . '.cat_id = aa.cat_id ' . ' WHERE (aa.cat_id IS NULL)';
        endif;
        if (!$result = $this->db->queryF($sql)) {
            //xoops_error("cleanOrphan error:". $sql);
        }
    ...
    public function updateTrack($category = null, $tracks = null)
    {
        if (empty($category)) {
