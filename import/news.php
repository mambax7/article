...
$sql = '   INSERT INTO '
       ...
       . '   FROM '
       . $GLOBALS['xoopsDB']->prefix('stories')
       . '       ORDER BY storyid ASC';

$result = $xoopsDB->queryF($sql);
xoops_result('succeed with building ARTICLE table: ' . $count = $xoopsDB->getAffectedRows());

$sql = '   INSERT INTO ' . art_DB_prefix('artcat') . '       (art_id,    cat_id,     uid,    ac_register,    ac_publish)' . '   SELECT ' . '       storyid,    topicid,    uid,    created,        published' . '   FROM ' . $GLOBALS['xoopsDB']->prefix('stories') . '       ORDER BY storyid ASC';
...
$sql = '   INSERT INTO '
       ...
       . '   FROM '
       . $GLOBALS['xoopsDB']->prefix('stories_votedata')
